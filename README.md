...
[GoCD agent](https://www.gocd.io) docker image based on fedora 30.
...
```
docker run -d -e GO_SERVER_URL=... gocd/gocd-agent-fedora-30:v19.4.0
```
...
```
docker run -d -e GO_SERVER_URL=https://$(docker inspect --format='{{(index (index .NetworkSettings.IPAddress))}}' angry_feynman):8154/go gocd/gocd-agent-fedora-30:v19.4.0
```
OR
...
```
docker run -d -e GO_SERVER_URL=https://<ip_of_host_machine>:$(docker inspect --format='{{(index (index .NetworkSettings.Ports "8154/tcp") 0).HostPort}}' angry_feynman)/go gocd/gocd-agent-fedora-30:v19.4.0
```
...
docker run -d \
        ...
        gocd/gocd-agent-fedora-30:v19.4.0
```
...
```
docker run -v /path/to/godata:/godata -v /path/to/home-dir:/home/go gocd/gocd-agent-fedora-30:v19.4.0
```
...
```
docker run -e GO_AGENT_SYSTEM_PROPERTIES="-Dfoo=bar" gocd/gocd-agent-fedora-30:v19.4.0
```
